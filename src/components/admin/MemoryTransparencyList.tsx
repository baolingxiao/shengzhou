      ...
      {records.map((row, i) => (
        ...
        >
          <div className="flex items-center justify-between gap-2">
            <span className="text-[10px] text-muted">{formatTime(row.ts)}</span>
          ...
          {row.reply_preview && (
            <p className="mt-1 text-xs leading-relaxed text-muted">
              <span className="text-muted/80">沈昼：</span>
              {row.reply_preview}
            ...
            {row.memory_ids.length === 0 ? (
              ...
              row.memory_ids.map((id) => (
                <button
                  ...
                  className={cn(
                    'rounded-full border border-[#07c160]/40 bg-[#07c160]/10',
                    'px-2 py-0.5 font-mono text-[10px] text-[#07c160] hover:bg-[#07c160]/20',
                  )}
                >
                  {id}
                </button>
